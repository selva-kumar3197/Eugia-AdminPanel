...
function ProductList() {
    ...
    const handleValidation = () => {
        ...
        const { image } = file
        if (!heading) {
            newErros.heading = 'please enter report name'
        } else if (heading && !textReg.test(heading)) {
            newErros.heading = 'numbers is not allowed'
        }
        else if (heading && heading.length > 50) {
            newErros.name = 'report name should be below 50 charecters'
        }

        if (!text) {
            newErros.text = 'please enter report date'
        }
        // else if (text && !textReg.test(text)) {
        //     newErros.text = 'numbers is not allowed'
        // }
        else if (text && text.length > 50) {
            newErros.text = 'report date should be below 50 charecters'
        }
        if (!image) {
    ...
    const handleSubmit = () => {
        ...
        if (updateMemberForm.text) {
            formData.append("report_date", updateMemberForm.text);
        }
